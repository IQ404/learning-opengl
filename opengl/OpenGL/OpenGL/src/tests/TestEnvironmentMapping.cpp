...
namespace Test
{
    TestEnvironmentMapping::TestEnvironmentMapping(const std::string& test_name)
        ...
        TestBase(test_name)
	{
        ...
        m_model_shader = std::make_unique<Shader>("res/shaders/EnvironmentMapping.shader");
	...
	void TestEnvironmentMapping::OnUpdate(float dt)
	{
        
	}

	void TestEnvironmentMapping::OnRender()
	{
        SkyboxPass();
        ModelPass();
	}
    ...
    void TestEnvironmentMapping::SkyboxPass()
    {
    ...
    void TestEnvironmentMapping::ModelPass()
    {
        m_projection_matrix = glm::perspective(glm::radians(m_fovy), 4.0f / 3.0f, 0.1f, 100.0f); // aspect ratio = 4:3
