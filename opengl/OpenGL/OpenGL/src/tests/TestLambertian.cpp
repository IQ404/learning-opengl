...
namespace Test
{
	TestLambertian::TestLambertian(const std::string& test_name)
        ...
        TestBase(test_name)
	{
        ...
        m_shader = std::make_unique<Shader>("res/shaders/Lambertian.shader");

        m_shader->Bind();
        m_shader->SetUniform_float_matrix_4_4("u_MVP", m_mvp_matrix);
        m_shader->SetUniform_float_matrix_4_4("u_Model", m_model_matrix);
        m_shader->SetUniform_1int("u_MixWithHemisphereLighting", 0);
        m_shader->SetUniform_1int("u_LinearTosRGBCorrection", 0);

        delete[] arr;
	}

	TestLambertian::~TestLambertian()
	{
	...
	void TestLambertian::OnUpdate(float dt)
	{
	...
	void TestLambertian::OnRender()
	{
        m_renderer.LinearDraw(*m_vao, *m_shader, m_count);
	}

	void TestLambertian::OnImGuiRender()
	{
        if (ImGui::Button("Mix with Hemisphere Lighting"))
        {
            m_mix_with_hemisphere_lighting = !m_mix_with_hemisphere_lighting;
            
            if (m_mix_with_hemisphere_lighting)
            {
                m_shader->SetUniform_1int("u_MixWithHemisphereLighting", 1);
            }
            else
            {
                m_shader->SetUniform_1int("u_MixWithHemisphereLighting", 0);
        ...
        if (ImGui::Button("Linear to sRGB Correction"))
        {
            m_linear_to_srgb_correction = !m_linear_to_srgb_correction;

            if (m_linear_to_srgb_correction)
            {
                m_shader->SetUniform_1int("u_LinearTosRGBCorrection", 1);
            }
            else
            {
                m_shader->SetUniform_1int("u_LinearTosRGBCorrection", 0);
