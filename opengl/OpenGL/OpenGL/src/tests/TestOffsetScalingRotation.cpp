#include "TestOffsetScalingRotation.h"
...
namespace Test
{
    TestOffsetScalingRotation::TestOffsetScalingRotation(const std::string& test_name)
        : m_model_world_coordinates{ 0.0f,0.0f,-15.0f },
        TestBase(test_name)
	{
        ...
        m_model_shader = std::make_unique<Shader>("res/shaders/OffsetScalingRotation.shader");
	...
	TestOffsetScalingRotation::~TestOffsetScalingRotation()
	{
	...
	void TestOffsetScalingRotation::OnUpdate(float dt)
	{
        m_model_shader->Bind();
	...
	void TestOffsetScalingRotation::OnRender()
	{
        SkyboxPass();
        ModelPass();
	}

	void TestOffsetScalingRotation::OnImGuiRender()
	{
    ...
    void TestOffsetScalingRotation::SkyboxPass()
    {
    ...
    void TestOffsetScalingRotation::ModelPass()
    {
        m_view_matrix = 
            glm::rotate(glm::mat4{ 1.0f }, -glm::radians(m_camera_vertical_rotation), glm::vec3(1, 0, 0)) *
            glm::rotate(glm::mat4{ 1.0f }, glm::radians(m_camera_horizontal_rotation), glm::vec3(0, 1, 0));
        m_projection_matrix = glm::perspective(glm::radians(m_fovy), 4.0f / 3.0f, 0.1f, 100.0f); // aspect ratio = 4:3
        m_model_matrix = glm::translate(glm::mat4{ 1.0f }, m_model_world_coordinates) * glm::rotate(glm::mat4{ 1.0f }, glm::radians(m_model_y_rotation), glm::vec3(0, 1, 0));
        m_model_shader->Bind();
