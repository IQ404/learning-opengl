#include "TestTVScanLines.h"
#include "DebugTools.h"
#include "imgui/imgui.h"

namespace Test
{
	TestTVScanLines::TestTVScanLines(const std::string& test_name)
        ...
        TestBase(test_name)
	{
        ...
        m_shader = std::make_unique<Shader>("res/shaders/TVScanLines.shader");
        ...
        m_shader->SetUniform_1float("u_TimePassed", 0);
	...
	TestTVScanLines::~TestTVScanLines()
	{
	...
	void TestTVScanLines::OnUpdate(float dt)
	{
        // We are hacking the delta time using ImGui and right now we are not using dt
        time_passed += 1.0f / ImGui::GetIO().Framerate;
        m_shader->SetUniform_1float("u_TimePassed", time_passed);
        //std::cout << time_passed << std::endl;
	}

	void TestTVScanLines::OnRender()
	{
        m_renderer.Draw(*m_vao, *m_index_buffer, *m_shader);
	}

	void TestTVScanLines::OnImGuiRender()
	{
