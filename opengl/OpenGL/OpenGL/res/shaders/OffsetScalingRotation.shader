...
void main()
{
    ...
    local_space_position.x *= remap(sin(5.0 * u_TimePassed), -1.0, 1.0, 1.0, 1.8);
    // right now the local normal is not corrected with respect to the model's scaling

    local_space_position = rotateY(0.5 * u_TimePassed) * local_space_position;
    local_space_normal = rotateY(0.5 * u_TimePassed) * normal;

    local_space_position.y += 2.5 * sin(u_TimePassed);
